...
async function Solving(naze, store) {
	...
	naze.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: naze.user.id, quoted: m.quoted && m.quoted.fakeObj, ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 });
		apb.key = m.key
...
async function Serialize(naze, msg, store, groupCache) {
	...
	if (m.key) {
		...
		if (m.isGroup) {
			...
			if (metadata.addressingMode === 'lid') {
				m.key.participant = m.sender = metadata.participants.find(a => a.lid === m.sender)?.id || m.sender;
			}
	...
	if (m.message) {
		...
		m.expiration = m.msg?.contextInfo?.expiration || store?.messages?.[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0
